          ...
          <article key={slide.id} className="grid gap-6 py-8 lg:grid-cols-[minmax(18rem,0.9fr)_minmax(24rem,1.1fr)] lg:gap-10">
            <div>
              ...
              <div className="mt-3 grid gap-2 sm:grid-cols-2">
                <label className="inline-flex min-h-11 cursor-pointer items-center justify-center gap-2 border border-ink px-4 text-sm font-semibold hover:bg-ink hover:text-white">
                ...
                {slide.mediaType === "video" && (
                  <label className="inline-flex min-h-11 cursor-pointer items-center justify-center gap-2 border border-ink/30 px-4 text-sm font-semibold hover:border-ink">
              ...
              <p className="mt-2 text-xs leading-relaxed text-ink/60">
                Images: JPG, PNG, WebP or AVIF up to 6 MB. Videos: MP4, M4V, WebM or MOV up to 80 MB.
              </p>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between gap-4 border-b border-ink/15 pb-4">
                ...
                <div className="flex gap-1">
                  ...
                  <button type="button" onClick={() => setSlides((current) => current.filter((_, itemIndex) => itemIndex !== index))} disabled={slides.length === 1} className="grid h-11 w-11 place-content-center border border-ink/25 text-red-800 disabled:cursor-not-allowed disabled:opacity-30" aria-label={`Delete slide ${index + 1}`}><Trash aria-hidden="true" /></button>
                </div>
              </div>
              ...
              <div className="mt-5 grid gap-5">
                <label className="grid gap-2 text-sm font-semibold">
                  Heading
                  <input value={slide.title} maxLength={90} onChange={(event) => updateSlide(index, { title: event.target.value })} className="min-h-12 border border-ink/30 bg-white px-4 font-normal outline-none focus:border-ink" />
                </label>
                <label className="grid gap-2 text-sm font-semibold">
                  Description
                  <textarea value={slide.copy} maxLength={240} rows={3} onChange={(event) => updateSlide(index, { copy: event.target.value })} className="resize-y border border-ink/30 bg-white px-4 py-3 font-normal leading-relaxed outline-none focus:border-ink" />
                </label>
                {slide.mediaType === "image" && (
                  <label className="grid gap-2 text-sm font-semibold">
                    ...
                    <input value={slide.alt} maxLength={160} onChange={(event) => updateSlide(index, { alt: event.target.value })} className="min-h-12 border border-ink/30 bg-white px-4 font-normal outline-none focus:border-ink" />
                  </label>
                )}
                <label className="grid max-w-56 gap-2 text-sm font-semibold">
                  Time on screen
                  <select value={slide.durationMs} onChange={(event) => updateSlide(index, { durationMs: Number(event.target.value) })} className="min-h-12 border border-ink/30 bg-white px-4 font-normal outline-none focus:border-ink">
      ...
      <div className="flex flex-col gap-3 py-6 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <button type="button" onClick={() => void save()} disabled={!dirty || saving || uploading !== null} className="inline-flex min-h-12 items-center justify-center gap-2 bg-ink px-6 text-sm font-semibold text-white hover:bg-walnut disabled:cursor-not-allowed disabled:opacity-40">
            <FloppyDisk aria-hidden="true" /> {saving ? "Publishing…" : "Save changes"}
          </button>
        </div>
      </div>
    </section>
